...
import type { DBCertificate, DBCertificateDocument } from "@/types/portfolioTypes"
...
export async function getCertificates(): Promise<DBCertificate[]> {
  try {
    ...
    const certificatesCollection = db.collection<DBCertificateDocument>("certificates")
    ...
    throw new Error(`Error fetching certificates: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
export async function addCertificate(formData: FormData) {
  try {
    ...
    throw new Error(`Error adding certificate: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
export async function updateCertificate(id: string, formData: FormData) {
  try {
    ...
    await certificatesCollection.updateOne({ _id: new ObjectId(id) }, { $set: updateData })

    return { _id: id, ...updateData }
  } catch (error) {
    console.error("Error updating certificate:", error)
    throw new Error(`Error updating certificate: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
export async function deleteCertificate(id: string) {
  try {
    ...
    throw new Error(`Error deleting certificate: ${error instanceof Error ? error.message : String(error)}`)
  }
}
