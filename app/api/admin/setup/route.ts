  ...
  try {
    ...
    if (!expectedPin) {
      return NextResponse.json(
        { message: "Admin setup is not configured. Please set ADMIN_SETUP_PIN environment variable." },
        { status: 500 },
      )
    }

    if (validatedData.secretPin !== expectedPin) {
      return NextResponse.json({ message: "Invalid secret PIN" }, { status: 403 })
    }
    ...
    const avatarValue = validatedData.avatar && validatedData.avatar.trim() !== "" ? validatedData.avatar : null
