  ...
  useEffect(() => {
    ...
    const getRelativeTimeString = (isoString: string): string => {
      ...
      switch (unit) {
        ...
        case "day":
        case "days":
          return Number.parseInt(value) <= 6 ? `${value} day${value === "1" ? "" : "s"} ago` : distance
        case "week":
        case "weeks":
          return Number.parseInt(value) <= 4 ? `${value} week${value === "1" ? "" : "s"} ago` : distance
        case "month":
        case "months":
          return Number.parseInt(value) <= 12 ? `${value} month${value === "1" ? "" : "s"} ago` : distance
        case "year":
        case "years":
